...
const AdminDashboard = () => {
  ...
  const fundingData = [{ name: "Funding", value: totalFunding / 100 }];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        <Card
          icon={<FaUsers fill="yellow" />}
          title="Total Users"
          value={totalUsers}
        />
        <Card
          icon={<FaDonate fill="green" />}
          title="Total Funding ($)"
          value={`${(totalFunding / 100).toFixed(2)}`}
        />
        <Card
          icon={<FaTint />}
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={usersData}>
              ...
              <YAxis />
              <Tooltip />
              <Bar dataKey="value" fill="#f87171" barSize={50} />
            </BarChart>
          </ResponsiveContainer>
        </div>
